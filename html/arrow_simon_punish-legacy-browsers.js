...
let expName = 'arrow_simon_punish';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  punish_cue = new visual.TextStim({
    win: psychoJS.window,
    name: 'punish_cue',
    text: '',
    font: 'Arial',
  ...
  complete_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'complete_text',
    text: '',
    font: 'Arial',
  ...
  complete_resp = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
...
function trial_train_simonRoutineEachFrame(snapshot) {
  return function () {
    ...
    if (trial_resp_simon.status === PsychoJS.Status.STARTED) {
      let theseKeys = trial_resp_simon.getKeys({keyList: ['w', 'p', 'esc'], waitRelease: false});
...
function trialRoutineBegin(snapshot) {
  return function () {
    ...
    target_onset = 1.5 + Math.random();
    punish_text = ("-$" + punish.toString());
    // keep track of which components have finished
    trialComponents = [];
    trialComponents.push(punish_cue);
...
function trialRoutineEachFrame(snapshot) {
  return function () {
    ...
    // *punish_cue* updates
    if (t >= 0.0 && punish_cue.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      punish_cue.tStart = t;  // (not accounting for frame time here)
      punish_cue.frameNStart = frameN;  // exact frame index
      
      punish_cue.setAutoDraw(true);
    }

    frameRemains = 0.0 + 0.6 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (punish_cue.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      punish_cue.setAutoDraw(false);
    }
    
    if (punish_cue.status === PsychoJS.Status.STARTED){ // only update if being drawn
      punish_cue.setText(punish_text, false);
    }
    ...
    if (trial_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = trial_resp.getKeys({keyList: ['w', 'p', 'esc'], waitRelease: false});
...
function completionRoutineBegin(snapshot) {
  return function () {
    ...
    rew_msg = (("You earned a cash bonus of " + rand_rew.toString()) + " US dollars. ");
    ...
    completionComponents.push(complete_text);
    completionComponents.push(complete_resp);
...
function completionRoutineEachFrame(snapshot) {
  return function () {
    ...
    // *complete_text* updates
    if (t >= 0.0 && complete_text.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      complete_text.tStart = t;  // (not accounting for frame time here)
      complete_text.frameNStart = frameN;  // exact frame index
      
      complete_text.setAutoDraw(true);
    }

    
    if (complete_text.status === PsychoJS.Status.STARTED){ // only update if being drawn
      complete_text.setText(complete_msg, false);
    }
    
    // *complete_resp* updates
    if (t >= 0.0 && complete_resp.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      complete_resp.tStart = t;  // (not accounting for frame time here)
      complete_resp.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { complete_resp.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { complete_resp.start(); }); // start on screen flip
    }

    if (complete_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = complete_resp.getKeys({keyList: ['space'], waitRelease: false});
      _complete_resp_allKeys = _complete_resp_allKeys.concat(theseKeys);
      if (_complete_resp_allKeys.length > 0) {
        complete_resp.keys = _complete_resp_allKeys[_complete_resp_allKeys.length - 1].name;  // just the last key pressed
        complete_resp.rt = _complete_resp_allKeys[_complete_resp_allKeys.length - 1].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function completionRoutineEnd(snapshot) {
  return function () {
    ...
    psychoJS.experiment.addData('complete_resp.keys', complete_resp.keys);
    if (typeof complete_resp.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('complete_resp.rt', complete_resp.rt);
        routineTimer.reset();
        }
